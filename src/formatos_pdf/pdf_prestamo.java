 ...
 * @author ROBERT
 */
public class pdf_prestamo extends PdfPageEventHelper {
  ...
  public static void dato(String ruta,Integer prestamo,String Modo) throws DocumentException, MalformedURLException, IOException, SQLException {
     Integer numero = (int) (Math.random() * 10000000) + 1;
      try {
         
        buscar_sql(1,prestamo);
        buscar_sql(2,prestamo);
        buscar_sql(3,prestamo);
        
        conexion cnn = new conexion(sql,"buscar");  
        // 
           FileOutputStream archivo = new FileOutputStream(ruta + "/doc_prestamo" + numero + ".pdf");  
           ...
           // Protocolo de Página
             ...
             pdf_prestamo event = new pdf_prestamo();
             writer.setPageEvent(event);
        
              doc.open();
              doc.addTitle("PRÉSTAMO DE MATERIALES" );
       ...
       if (cnn.estado){       
        ...
        // Texto Departamento
        /*    info = new PdfPCell(new Paragraph("Cedula Solicitante: " + cnn.rs.getInt(10)));
        ...
        // Texto Documento
        info = new PdfPCell(new Paragraph("Nombres: " + cnn.rs.getString(20)));
        ...
        item1.addCell(info);
         ...
         info = new PdfPCell(new Paragraph("Clase de Movimiento: P101"));
        info.setHorizontalAlignment(Element.ALIGN_LEFT);
        info.setBorderWidthBottom(0.0F);
        ...
        info = new PdfPCell(new Paragraph("Nro de Préstamo: P-" + prestamo ));
        info.setHorizontalAlignment(Element.ALIGN_LEFT);
    ...
    // --------------------------------------------------------------------
            ...
            float[] medidaCeldas1 = {0.3f,1.4f,0.5f,0.6f,0.4f,0.4f,0.6f,0.6f};
            ...
            table.setWidthPercentage(100);
                ...
                info = new PdfPCell(new Phrase("FECHA SALIDA",FontFactory.getFont("arial",9,BaseColor.BLACK)));
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);
                
                info = new PdfPCell(new Phrase("FECHA DESDE",FontFactory.getFont("arial",9,BaseColor.BLACK)));
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);
                
                info = new PdfPCell(new Phrase("FECHA HASTA",FontFactory.getFont("arial",9,BaseColor.BLACK)));
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);
              ...
              while(cnn.rs.next()){   
                ...
                // Proyecto
                info = new PdfPCell(new Paragraph(cnn.rs.getString(16) + "",FontFactory.getFont("arial",8,BaseColor.GRAY)));
                info.setBorder(0);
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);
                
                 //  CANTIDAD
                info = new PdfPCell(new Paragraph(cnn.rs.getInt(18) + "",FontFactory.getFont("arial",8,BaseColor.GRAY)));
                info.setBorder(0);
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);
                
                //  UMB
                info = new PdfPCell(new Paragraph(cnn.rs.getString(5) + "",FontFactory.getFont("arial",8,BaseColor.GRAY)));
                info.setBorder(0);
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);
            
                //  fecha salida
                info = new PdfPCell(new Paragraph(cnn.rs.getString(21) + "",FontFactory.getFont("arial",8,BaseColor.GRAY)));
                info.setBorder(0);
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);
            
                // fecha desde
                info = new PdfPCell(new Paragraph(cnn.rs.getString(22) + "",FontFactory.getFont("arial",8,BaseColor.GRAY)));
                info.setBorder(0);
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);

                // fecha hasta
                info = new PdfPCell(new Paragraph(cnn.rs.getString(23) + "",FontFactory.getFont("arial",8,BaseColor.GRAY)));
                info.setBorder(0);
                info.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(info);
        ...
        // User
         info = new PdfPCell(new Phrase("USUARIO: " + cnn.rs.getString(1) + "" ,FontFactory.getFont("arial",6,BaseColor.GRAY)));
         info.setColspan(2);
         info.setBorder(0);
        ...
        // HORA
         ...
         doc.add(item1);
          ...
          Logger.getLogger(pdf_prestamo.class.getName()).log(Level.SEVERE, null, ex);
      }
           ...
           File archivo_abrir = new File(ruta + "/doc_prestamo" + numero + ".pdf");
 ...
 // ENCABEZADO
  public void onStartPage(PdfWriter writer,Document document) {
    ...
    public static void buscar_sql(Integer consulta,Integer prestamo){
        
        switch(consulta){
            
            case 1: sql = "SELECT id_prestamo, fk_material, descripcionmaterial, numero_material, codigo_umb, fk_empresa, descripcionempresa, fk_area, descripcionarea, fk_almacen, descripcionalmacen, fk_ubicacion, descripcionubicacion, fk_movimiento, fk_proyecto, descripcionproyecto, posicion, cantidad_prestada, nro_documento, nombresolicitante, fecha_solicitud, fecha_desde, fecha_hasta, justificacion, fecha_recepcion, dano, foto, descripcion_dano, id_entrada, status ";
                    sql += " FROM vw_prestamo";
                    sql += " WHERE nro_documento = " + prestamo;
                    break;
                    
            case 2: sql2 = "SELECT DISTINCT fecha_solicitud FROM vw_prestamo ";
                    sql2 += " WHERE nro_documento = " + prestamo + ";";
                    break;  
                    
             case 3: sql3 = "SELECT DISTINCT nombresolicitante FROM vw_prestamo ";
                    sql3 += " WHERE nro_documento = " + prestamo + ";";
                    break;          
